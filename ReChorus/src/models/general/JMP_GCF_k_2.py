...
class LightGCNBase(object):
	...
	def build_adjmat(user_count, item_count, train_mat, selfloop_flag=False):
		...
		def normalized_adj_single(adj):
			...
			d_inv_04 = np.power(rowsum, -0.3).flatten()    # 计算 D^(-0.3)
			
			d_inv_sqrt[np.isinf(d_inv_sqrt)] = 0.
			d_inv_04[np.isinf(d_inv_04)] = 0.

			d_mat_inv_sqrt = sp.diags(d_inv_sqrt)  # D^-1/2
			d_mat_inv_04 = sp.diags(d_inv_04)      # D^(-0.3)

			bi_lap = d_mat_inv_sqrt.dot(adj).dot(d_mat_inv_04)  # D^-1/2 * A * D^(-0.3)
			return bi_lap.tocoo()
...
class JMP_GCF_k_2(GeneralModel, LightGCNBase):
	reader = 'BaseReader'
	runner = 'BaseRunner'
