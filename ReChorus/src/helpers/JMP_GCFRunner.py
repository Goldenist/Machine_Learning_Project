...
class JMP_GCFRunner(object):
	...
	def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
		...
		for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
			...
			out_dict = model(batch) # 返回 user_embeddings, item_embeddings
			loss = 0.0
			if epoch / self.epoch <= 0.3:
				cur_out_dict = out_dict[0]
				prediction = cur_out_dict['prediction']
				if len(prediction.shape)==2:
					...
					cur_out_dict['prediction'] = restored_prediction
				loss += model.loss(cur_out_dict)
				loss.backward(retain_graph=True)
				model.optimizer.step()
			if epoch / self.epoch <= 0.6:
				cur_out_dict = out_dict[1]
				prediction = cur_out_dict['prediction']
				if len(prediction.shape)==2:
					restored_prediction = torch.zeros(*prediction.shape).to(prediction.device)
					restored_prediction[torch.arange(item_ids.shape[0]).unsqueeze(-1), indices] = prediction   
	...
	def predict(self, dataset: BaseModel.Dataset, save_prediction: bool = False) -> np.ndarray:
		...
		for batch in tqdm(dl, leave=False, ncols=100, mininterval=1, desc='Predict'):
			if hasattr(dataset.model,'inference'):
				prediction_list = dataset.model.inference(utils.batch_to_gpu(batch, dataset.model.device))
				prediction = 0
				for cur_predict in prediction_list:
					prediction += cur_predict['prediction']
			else:
				prediction_list = dataset.model(utils.batch_to_gpu(batch, dataset.model.device))
				prediction = 0
				for cur_predict in prediction_list:
					prediction += cur_predict['prediction']
